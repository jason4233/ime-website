...
const acts = [
  {
    ...
    title: "所有的「還可以」，\n都是對自己的一種將就。",
    body: `我們見過太多女人，在 25 歲以後開始練習一件事：妥協。
...
它把「修復」這個指令，精準送到該去的地方。
而臍帶間質幹細胞分泌的外泌體，是郵差裡最溫柔、最聰明的那一種。
      ...
      <div className="relative z-10 h-full flex items-center justify-center px-6">
        <div className="max-w-3xl mx-auto text-center">
          <AnimatePresence mode="wait">
            ...
            >
              {/* 幕數 */}
              <p className={`font-display text-sm tracking-[0.3em] mb-8 ${act.subColor}`}>
                {String(activeAct + 1).padStart(2, "0")} / 03
              </p>

              {/* 標題 */}
              <h2 className={`font-serif-tc text-[clamp(1.6rem,4vw,2.8rem)] leading-[1.3] font-semibold mb-8 whitespace-pre-line ${act.textColor}`}>
                {act.title}
              </h2>
              ...
              <motion.div
                ...
                className={`w-12 h-px mx-auto mb-8 origin-center ${act.lineColor}`}
              />

              {/* 內文 */}
              <p className={`font-sans-tc text-body-lg leading-[2] whitespace-pre-line max-w-xl mx-auto ${act.subColor}`}>
                {act.body}
              </p>
            </motion.div>
          </AnimatePresence>
